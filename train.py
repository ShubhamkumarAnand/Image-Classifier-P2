...
def validation():               #validation function
...
def process_data(data_dir):             #process data
...
def build_model(data):              #Build Model
...
def test_accuracy(model,loader,device='cpu'):       #Accuracy Testing
    correct = 0
    total = 0
    with torch.no_grad():
        for data in loader:
...
def train(model,data):                  #Training model
